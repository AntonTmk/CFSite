...
<td><a href='review.php'><font color='white' size='5'>Відгуки</a></td></tr></table>
...
<tr>
	...
	$mysqli->query ("SET NAMES utf8'");
			...
			$mi = $mysqli->query("SELECT photo FROM myloom.cat where id = $num");
			$result = mysqli_fetch_row($mi);
echo "<tr><td ><img src=$result[0] width='400px''></td>";
			
			$mi = $mysqli->query("SELECT name FROM myloom.cat where id = $num");
			$result = mysqli_fetch_row($mi);
echo "<td width='500px'><font color='white' size='5'>$result[0]</td>";
			
			$mi = $mysqli->query("SELECT cost FROM myloom.cat where id = $num");
			$result = mysqli_fetch_row($mi);
echo "<td width='200px'><font color='white' size='5'><p>$result[0]грн</p></td></tr>";
				$num=$num+1;
	...
	?>
<td><button class="btn btn-outline-success">follow</button></td></tr>
</table>
